...
//
// Auth: custom x-token header. Invoked by pg_cron via net.http_post.
// SB-440 (2026-09-15): the expected token used to be a literal in this file. The same literal
// was in two database functions, in four cron job commands, and in
// supabase/migrations/20260619021842_schedule_agent_runner.sql — which is committed and pushed
// to a PUBLIC GitHub repository, so it was world-readable. It has been rotated and now lives in
// Vault. This function no longer holds it: it asks the database whether the presented token
// matches, via public.agent_runner_token_matches(), which is EXECUTE-granted to service_role
// only and never returns the secret. Do not reintroduce a literal here.
...
// Dispatch is scopeable via body.dispatchProjects (list of project ids), guarded once-per-ticket
// (a work item with any prior run is never re-dispatched), and body.forceItems (ticket_codes) can
// re-kick specific tickets after a blocker clears. Candidates ordered by sort_order.
//
// SB-390 (2026-08-31): this function used to gate on meta.dev_automation while every board read
// projects.automation_status. The two disagreed on 17 of 41 projects — five of them running while
// the board showed them paused — and a QA check written against the column recorded a false defect
// because of it. automation_status is now the single authoritative flag and meta.dev_automation has
// been dropped. Both fields were reconciled to agree before this change, so the switch was inert.
...
Deno.serve(async (req: Request) => {
  ...
  // SB-440: validate against the Vault-held token without ever receiving its value.
  {
    const presented = req.headers.get("x-token") ?? "";
    const { data: tokenOk, error: tokenErr } = await sb.rpc("agent_runner_token_matches", { p_token: presented });
    if (tokenErr || tokenOk !== true) return new Response("forbidden", { status: 403 });
  }
  ...
  // ---- Active automated projects (SB-390: automation_status, the one authoritative flag) ----
  const { data: projs } = await sb.from("projects").select("id,name,user_id,automation_status,archived").eq("archived", false);
  const activeProjects = (projs ?? []).filter((p: any) => p?.automation_status === "active");
  const projIds = activeProjects.map((p: any) => p.id);
  const userOf = (pid: string) => activeProjects.find((p: any) => p.id === pid)?.user_id ?? null;
  if (!projIds.length) { (out as any).note = "no projects with automation_status=active"; return json(out); }
  ...
  if (dispatchEnabled) {
    ...
    if (!apiKey) {
      ...
      for (const it of (ready ?? []) as any[]) {
        ...
        if (!ag) continue;
        // SB-393: an agent with no authored prompt would otherwise be dispatched with nothing
        // but the generic runner boilerplate, producing plausible output with no role, no
        // boundaries and no escalation path. Refuse rather than improvise.
        if (!(ag.system_prompt ?? "").trim()) {
          skipped.push({ id: it.id, title: it.title, agent: ag.name, reason: "agent has an empty system_prompt — not dispatched (SB-393)" });
          continue;
        }
