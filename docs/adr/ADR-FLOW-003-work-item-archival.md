...
| **Excluded** | (1) rows with `completed_at IS NULL` — 62 today, see §4.2; (2) `type = 'epic'` with at least one non-`done` child — archiving a live epic's container hides active work; (3) any row with a non-`done` child — **5 today**. No blocker exclusion is needed: `blocked_by_count` counts a link only when `blocker.status <> 'done'`, so a `done` candidate is by construction not blocking anything, and hiding it cannot change any live ticket's count. |
